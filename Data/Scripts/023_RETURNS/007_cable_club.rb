...
def pbCableClub
  Kernel.pbMessage(_INTL("\\c[1]Bonjour, bienvenue au Cable Club."))
  if $Trainer.party.length == 0
    Kernel.pbMessage(_INTL("\\c[1]Je suis désolé, vous devez avoir un Pokémon pour accéder au Cable Club."))
    return
  end
  msgwindow = Kernel.pbCreateMessageWindow()
  begin
    Kernel.pbMessageDisplay(msgwindow, _ISPRINTF("\\c[1]Pour commencer, vous devez nous fournir l'ID du Dresseur que vous souhaitez contacter."))
    Kernel.pbMessageDisplay(msgwindow, _ISPRINTF("Veuillez saisir l'ID du dresseur.\\n(Votre ID de Dresseur est : {1:05d})\\^", $Trainer.public_id($Trainer.id)))
    partner_trainer_id = ""
    loop do
      ...
      Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Je suis désolé, {1} n'est pas un ID de Dresseur.\\nUn ID de Dresseur n'est composé que de chiffres.\\nIl est disponible sur la Carte de Dresseur.", partner_trainer_id))
    end
    ...
    case e.message
    when "disconnected"
      Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Merci de votre fidélité au Cable Club.\\nNous espérons vous revoir bientôt."))
      return true
    when "invalid party"
      Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Je suis désolé, votre équipe compte des Pokémons interdits par le Cable Club."))
      Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Revenez plus tard avec une autre équipe."))
      return false
    when "peer disconnected"
      Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Je suis désolé, l'autre dresseur s'est déconnecté."))
      return true
    else
      Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Je suis désolé, les serveurs du Cable Club ne fonctionnent pas actuellement."))
      Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Revenez plus tard."))
      return false
    end
  rescue Errno::ECONNREFUSED
    Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Je suis désolé, les serveurs du Cable Club sont en panne."))
    Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Revenez plus tard."))
    return false
  rescue
    pbPrintException($!)
    Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Je suis désolé, le Cable Club ne fonctionne pas actuellement."))
    Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Revenez plus tard."))
    return false
  ensure
...
module CableClub
  ...
  def self.connect_to(msgwindow, partner_trainer_id)
    pbMessageDisplayDots(msgwindow, _INTL("Connexion"), 0)
    Connection.open(HOST, PORT) do |connection|
      ...
      loop do
        ...
        if Input.press?(Input::B)
          message = case state
                    when :await_server; _INTL("Interrompre?\\^")
                    when :await_partner; _INTL("Interrompre la recherche?\\^")
                    else; _INTL("Se déconnecter?\\^")
                    end
          Kernel.pbMessageDisplay(msgwindow, message)
          return if Kernel.pbShowCommands(msgwindow, [_INTL("Oui"), _INTL("Non")], 2) == 0
        end
        ...
        when :await_server
          if connection.can_send?
            ...
            pbMessageDisplayDots(msgwindow, _ISPRINTF("Votre ID: {1:05d}\\nConnexion",$Trainer.publicID($Trainer.id)), frame)
          end

          # Waiting to be connected to the partner.
        when :await_partner
          pbMessageDisplayDots(msgwindow, _ISPRINTF("Votre ID: {1:05d}\\nRecherche",$Trainer.publicID($Trainer.id)), frame)
          connection.update do |record|
            ...
            when :found
              ...
              Kernel.pbMessageDisplay(msgwindow, _INTL("{1} est connecté!", partner_name))
              if client_id == 0
              ...
              raise "Message inconnu: #{type}"
            end
          end

          # Choosing an activity (leader only).
        when :choose_activity
          Kernel.pbMessageDisplay(msgwindow, _INTL("\\c[1]Veuillez choisir une activité.\\^"))
          command = Kernel.pbShowCommands(msgwindow, [_INTL("Combat Simple"), _INTL("Combat Double"), _INTL("Echange")], -1)
          case command
          when 0..1 # Battle
        ...
        when :await_accept_activity
          pbMessageDisplayDots(msgwindow, _INTL("En attente de l'acceptation de {1}", partner_name), frame)
          connection.update do |record|
            case (type = record.sym)
            when :ok
              case activity
              when :battle
                ...
                partner = GameData::Player.initialize(partner_name, trainertype)
            ...
            when :cancel
              Kernel.pbMessageDisplay(msgwindow, _INTL("Je suis désolé, {1} ne veut pas faire l'activité #{activity.to_s}.", partner_name))
              state = :choose_activity
        ...
        when :await_choose_activity
          pbMessageDisplayDots(msgwindow, _INTL("{1} choisit une activité", partner_name), frame)
          connection.update do |record|
            case (type = record.sym)
            when :battle
              seed = record.int
              ...
              if battle_type == :double && $Trainer.party.length < 2
                ...
                Kernel.pbMessageDisplay(msgwindow, _INTL("{1} veut se battre!\\^", partner_name))
                if Kernel.pbShowCommands(msgwindow, [_INTL("Oui"), _INTL("Non")], 2) == 0
                  connection.send do |writer|
                    writer.sym(:ok)
                    writer.int($Trainer.trainertype)
                  end
            ...
            when :trade
              Kernel.pbMessageDisplay(msgwindow, _INTL("{1} veut faire un échange!\\^", partner_name))
              if Kernel.pbShowCommands(msgwindow, [_INTL("Oui"), _INTL("Non")], 2) == 0
                connection.send do |writer|
                  writer.sym(:ok)
                end
        ...
        when :await_trade_pokemon
          ...
          connection.update do |record|
            case (type = record.sym)
            when :ok
              partner = GameData::Player.initialize(partner_name, $Trainer.trainertype)
  ...
  def self.parse_pkmn(record)
    species = record.int
    level = record.int
    pkmn = GameData::Pokemon.initialize(species, level, $Trainer)
    ...
    for i in 0...record.int
      pkmn.moves[i] = GameData::Pokemon::Move.initialize(record.int)
...
class PokeBattle_CableClub < PokeBattle_Battle
  ...
  def initialize(connection, client_id, scene, opponent_party, opponent)
    ...
    @client_id = client_id
    player = GameData::Player.initialize($Trainer.name, $Trainer.trainertype)
...
if defined?(ESSENTIALS_VERSION) && ESSENTIALS_VERSION =~ /^18/
  ...
  class PokeBattle_Battler
    alias old_pbFindUser pbFindUser
