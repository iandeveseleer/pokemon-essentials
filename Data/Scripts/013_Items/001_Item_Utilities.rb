...
def pbLearnMove(pkmn, move, ignore_if_known = false, by_machine = false, &block)
  ...
  if pkmn.hasMove?(move)
    pbMessage(_INTL("{1} connaît déjà {2}.", pkmn_name, move_name), &block) if !ignore_if_known
    return false
  elsif pkmn.numMoves < Pokemon::MAX_MOVES
    pkmn.learn_move(move)
    pbMessage(_INTL("\\se[]{1} apprend {2}!\\me[Niveau]", pkmn_name, move_name), &block)
    return true
  end
  pbMessage(_INTL("{1} veut apprendre {2}, mais il connait déjà {3} attaques.\1",
                  pkmn_name, move_name, pkmn.numMoves.to_word), &block)
  if pbConfirmMessage(_INTL("{1} doit oublier une attaque pour apprendre {2}?", pkmn_name, move_name), &block)
    loop do
      ...
      if move_index >= 0
        ...
        pbMessage(_INTL("{1} oublie {2}.\\nEt...\1", pkmn_name, old_move_name), &block)
        pbMessage(_INTL("\\se[]{1} apprend {2}!\\me[Niveau]", pkmn_name, move_name), &block)
        pkmn.changeHappiness("machine") if by_machine
        return true
      elsif pbConfirmMessage(_INTL("Arrêter d'apprendre {1}?", move_name), &block)
        pbMessage(_INTL("{1} n'a pas appris {2}.", pkmn_name, move_name), &block)
        return false
      end
    end
  else
