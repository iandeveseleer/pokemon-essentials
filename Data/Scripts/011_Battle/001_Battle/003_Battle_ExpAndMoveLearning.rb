class Battle
  ...
  def pbLearnMove(idxParty, newMove)
    ...
    pbDisplayPaused(_INTL("{1} veut apprendre {2}, mais il a déjà 4 attaques.",
                          pkmnName, moveName, pkmn.numMoves.to_word))    if pbDisplayConfirm(_INTL("Oublier une attaque pour apprendre {1}?",moveName))
    if pbDisplayConfirm(_INTL("Oublier une attaque pour apprendre {1}?", moveName))
      loop do
        ...
        if forgetMove >= 0
          ...
          pbDisplayPaused(_INTL("1, 2, &... ... ... Ta-da!")) { pbSEPlay("Battle ball drop") }
