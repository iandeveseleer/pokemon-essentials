...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    ypos = rect.y
    if index == count - 1
      textpos.push([_INTL("ANNULER"), rect.x, ypos + 2, false, self.baseColor, self.shadowColor])
    else
      item = @stock[index]
  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["itemtextwindow"].text =
        (itemwindow.item) ? @adapter.getDescription(itemwindow.item) : _INTL("Quitter le shopping.")
    ...
    @sprites["moneywindow"].text = _INTL("Argent:\r\n<r>{1}", @adapter.getMoneyString)
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if GameData::Item.get(item).is_important?
        next if !pbConfirm(_INTL("Certainement. Tu en veux {1}. cela fera {2}$. OK?",
      ...
      if added == quantity
        ...
        pbDisplayPaused(_INTL("Tu n'as plus de place pour ça."))
      end
    end
    @scene.pbEndBuyScene
  end
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("{1}? Oh, non.\nJe ne peux pas acheter ça.", itemnameplural))
        next
      end
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(
          _INTL("{1}? Combien veux-tu en vendre?", itemnameplural), item, qty
        )
      end
      if qty == 0
        @scene.pbHideMoney
      ...
      price /= 2
      price *= qty
      if pbConfirm(_INTL("Je peux t'en donner {1}$.\nCela te conviendrais?", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("Vous avez vendu {1} pour {2}$.",
...
def pbPokemonMart(stock, speech = nil, cantsell = false)
  ...
  cmd = pbMessage(speech || _INTL("Bienvenue! Comment puis-je t'aider?"), commands, cmdQuit + 1)
  loop do
