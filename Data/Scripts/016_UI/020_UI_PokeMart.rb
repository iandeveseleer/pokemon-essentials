...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    ypos = rect.y
    if index == count-1
      textpos.push([_INTL("ANNULER"), rect.x, ypos - 4, false, self.baseColor, self.shadowColor])
    else
      item = @stock[index]
        ...
        if $PokemonBag
          if quantity>=10 && GameData::Item.get(item).is_poke_ball? && GameData::Item.exists?(:PREMIERBALL)
            if @adapter.addItem(GameData::Item.get(:PREMIERBALL))
              pbDisplayPaused(_INTL("J'ajoute une Honor Ball."))
            end
          end
        end
      end
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("{1}? Oh, non. Je ne peux pas acheter ça.",itemname))
        next
      end
      qty=@adapter.getQuantity(item)
      next if qty==0
      ...
      if pbConfirm(_INTL("Je peux vous en donner {1}$. Cela vous conviendrai?",price.to_s_formatted))
        ...
        if qty>1
          pbDisplayPaused(_INTL("{1} vend {2} {3} pour {4}$.",$Trainer.name, qty, itemname, price.to_s_formatted)) { pbSEPlay("Mart buy item") }
        else
          pbDisplayPaused(_INTL("{1} vend 1 {2} pour {3}$.",$Trainer.name, itemname, price.to_s_formatted)) { pbSEPlay("Mart buy item") }
        end
          @scene.pbRefresh
      end
      @scene.pbHideMoney
