...
class PokemonTrade_Scene
  ...
  def pbTrade
    ...
    pbScene1
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("Pour le {2} de {1},\r\n{3} envoie {4}.\1",@trader1,speciesname1,@trader2,speciesname2)) { pbUpdate }
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("{1} dit adieu à {2}.",@trader2,speciesname2)) { pbUpdate }
    pbScene2
    pbMessageDisplay(@sprites["msgwindow"],
       _ISPRINTF("{1:s}\r\nID: {2:05d}   DO: {3:s}\1",
       @pokemon2.name,@pokemon2.owner.public_id,@pokemon2.owner.name)) { pbUpdate }
    pbMessageDisplay(@sprites["msgwindow"],
       _INTL("Prends soin de {1}.",speciesname2)) { pbUpdate }
  end
end
