...
=end
  ...
  def pbRefresh
    ...
    if @searchResults
      textpos.push([_INTL("Résultat de recherche"), 112, 314, 2, base, shadow])
      textpos.push([@dexlist.length.to_s, 112, 346, 2, base, shadow])
    else
      textpos.push([_INTL("Vus:"), 42, 314, 0, base, shadow])
      textpos.push([$player.pokedex.seen_count(pbGetPokedexRegion).to_s, 182, 314, 1, base, shadow])
      textpos.push([_INTL("Capturés:"), 42, 346, 0, base, shadow])
  ...
  def pbRefreshDexSearch(params, _index)
    ...
    # Write various bits of text
    textpos = [
      [_INTL("Mode de recherche"), Graphics.width / 2, 10, 2, base, shadow],
      [_INTL("Ordre"), 136, 64, 2, base, shadow],
      [_INTL("Nom"), 58, 122, 2, base, shadow],
      [_INTL("Type"), 58, 174, 2, base, shadow],
      [_INTL("Taille"), 58, 226, 2, base, shadow],
      [_INTL("Poids"), 58, 278, 2, base, shadow],
      [_INTL("Couleur"), 326, 122, 2, base, shadow],
      [_INTL("Forme"), 454, 174, 2, base, shadow],
      [_INTL("Reset"), 80, 346, 2, base, shadow, 1],
      [_INTL("Chercher"), Graphics.width / 2, 346, 2, base, shadow, 1],
      [_INTL("Annuler"), Graphics.width - 80, 346, 2, base, shadow, 1]
    ]
  ...
  def pbDexSearch
    ...
    @orderCommands = []
    @orderCommands[MODENUMERICAL] = _INTL("Numérique")
    @orderCommands[MODEATOZ]      = _INTL("A à Z")
    @orderCommands[MODEHEAVIEST]  = _INTL("Du plus lourd")
    @orderCommands[MODELIGHTEST]  = _INTL("Du plus léger")
    @orderCommands[MODETALLEST]   = _INTL("Du plus grand")
    @orderCommands[MODESMALLEST]  = _INTL("Du plus petit")
    ...
    loop do
      ...
      if Input.trigger?(Input::UP)
        ...
        when 8   # Start search (filter)
          ...
          if dexlist.length == 0
            pbMessage(_INTL("Aucun Pokémon correspondant trouvé."))
          else
            @dexlist = dexlist
            @sprites["pokedex"].commands = @dexlist
