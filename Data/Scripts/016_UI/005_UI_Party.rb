...
class PokemonPartyScreen
  ...
  def pbPokemonScreen
    ...
    loop do
      ...
      command_list.push(_INTL("ANNULER"))
      ...
      choice = @scene.pbShowCommands(_INTL("Que faire avec {1}?", pkmn.name), command_list)
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(move.id)
          ...
          if pbConfirmUseHiddenMove(pkmn, move.id)
            ...
            if move.id == :FLY
              ...
              @scene.pbStartScene(
                @party, (@party.length > 1) ? _INTL("Choisir un Pokémon.") : _INTL("Choisir un Pokémon ou annuler.")
              )
              next
            end
            return [pkmn, move.id]
...
MenuHandlers.add(:party_menu, :summary, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    screen.scene.pbSummary(party_idx) {
      screen.scene.pbSetHelpText((party.length > 1) ? _INTL("Choisir un Pokémon.") : _INTL("Choisir un Pokémon ou annuler."))
    }
  }
})
