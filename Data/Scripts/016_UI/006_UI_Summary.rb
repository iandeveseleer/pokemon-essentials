...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    textpos = [
      ...
      [_INTL("Objet"), 66, 324, 0, base, shadow]
    ]
    # Write the held item's name
    if @pokemon.hasItem?
      textpos.push([@pokemon.item.name, 16, 358, 0, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    else
      textpos.push([_INTL("Aucun"), 16, 358, 0, Color.new(192, 200, 208), Color.new(208, 216, 224)])
    end
  ...
  def drawPageOne
    ...
    # Write various bits of text
    textpos = [
      [_INTL("N. Dex"), 238, 86, 0, base, shadow],
      [_INTL("Espèce"), 238, 118, 0, base, shadow],
      [@pokemon.speciesName, 435, 118, 2, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("Type"), 238, 150, 0, base, shadow],
      [_INTL("DO"), 238, 182, 0, base, shadow],
      [_INTL("ID N."), 238, 214, 0, base, shadow]
    ]
    ...
    if @pokemon.shadowPokemon?
      ...
      textpos.push([_INTL("Points d'expérience"), 238, 246, 0, base, shadow])
      textpos.push([@pokemon.exp.to_s_formatted, 488, 278, 1, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      textpos.push([_INTL("Prochain N."), 238, 310, 0, base, shadow])
  ...
  def drawPageOneEgg
    ...
    # Write various bits of text
    textpos = [
      [_INTL("MEMO DRESSEUR"), 26, 22, 0, base, shadow],
      [@pokemon.name, 46, 68, 0, base, shadow],
      [_INTL("Objet"), 66, 324, 0, base, shadow]
    ]
    # Write the held item's name
    ...
    eggstate = _INTL("Qu'est-ce qui va éclore de ça ? Ça ne semble pas près d'éclore.") if @pokemon.steps_to_hatch < 10_200
  ...
  def drawPageTwo
    ...
    mapname = pbGetMapNameFromId(@pokemon.obtain_map)
    mapname = @pokemon.obtain_text if @pokemon.obtain_text && !@pokemon.obtain_text.empty?
    mapname = _INTL("Endroit Lointain") if nil_or_empty?(mapname)
    ...
    mettext = [_INTL("Recontré au niveau {1}.", @pokemon.obtain_level),
               ...
               "",
               _INTL("Rencontré malheureusement au niveau {1}.", @pokemon.obtain_level)][@pokemon.obtain_method]
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    # Write various bits of text
    textpos = [
      [_INTL("CAPACITES"), 26, 22, 0, base, shadow],
      [_INTL("CATEGORIE"), 20, 128, 0, base, shadow],
      [_INTL("PUISSANCE"), 20, 160, 0, base, shadow],
      [_INTL("PRECISION"), 20, 192, 0, base, shadow]
    ]
    imagepos = []
    # Write move names, types and PP amounts for each known move
    yPos = 104
  ...
  def drawPageFive
    ...
    # Write various bits of text
    textpos = [
      [_INTL("Rubans:"), 234, 338, 0, Color.new(64, 64, 64), Color.new(176, 176, 176)],
  ...
  def pbMarking(pokemon)
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          [_INTL("Marquer {1}", pokemon.name), 366, 102, 2, base, shadow],
          [_INTL("OK"), 366, 254, 2, base, shadow],
          [_INTL("ANNULER"), 366, 304, 2, base, shadow]
        ]
  ...
  def pbOptions
    ...
    if !@pokemon.egg?
      ...
      commands[cmdPokedex = commands.length]  = _INTL("Voir dans le Pokédex") if $player.has_pokedex
    end
