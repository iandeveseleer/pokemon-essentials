...
def pbStorePokemon(pkmn)
  ...
  if $player.party_full?
    ...
    pbMessage(_INTL("{1} st envoyé dans la boîte \"{2}\"!", pkmn.name, box_name))
  else
...
def pbAddPokemon(pkmn, level = 1, see_form = true)
  ...
  species_name = pkmn.speciesName
  pbMessage(_INTL("{1} obtient {2}!\\me[Pkmn get]\\wtnp[80]\1", $player.name, species_name))
...
def pbAddForeignPokemon(pkmn, level = 1, owner_name = nil, nickname = nil, owner_gender = 0, see_form = true)
  ...
  pkmn.calc_stats
  if owner_name
    pbMessage(_INTL("\\me[Pkmn get]{1} reçoit un Pokémon de la part de {2}.\1", $player.name, owner_name))
  else
    pbMessage(_INTL("\\me[Pkmn get]{1} reçoit un Pokémon.\1", $player.name))
  end
  was_owned = $player.owned?(pkmn.species)
  $player.pokedex.set_seen(pkmn.species)
  $player.pokedex.set_owned(pkmn.species)
