...
def pbItemBall(item, quantity = 1)
  ...
  if $bag.add(item, quantity)   # If item can be picked up
    ...
    pbMessage(_INTL("Vous rangez {1} dans la <icon=bagPocket{2}> \\c[1]Poche à {3}\\c[0].",
...
def pbReceiveItem(item, quantity = 1)
  ...
  if $bag.add(item,quantity)   # If item can be added
    pbMessage(_INTL("Vous rangez \\c[1]{1}\\c[0] dans la <icon=bagPocket{2}> \\c[1]Poche à {3}\\c[0].",
                    itemname,pocket,PokemonBag.pocket_names[pocket - 1]))
    return true
  end
