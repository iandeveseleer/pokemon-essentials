...
def pbCheckHiddenMoveBadge(badge = -1, showmsg = true)
  ...
  pbMessage(_INTL("Désolé, un nouveau Badge est requis.")) if showmsg
  return false
end
...
def pbCut
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_CUT, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("Cet arbre a l'air de pouvoir être coupé."))
    return false
  end
  if pbConfirmMessage(_INTL("Cet arbre a l'air de pouvoir être coupé!\1"))
    $stats.cut_count += 1
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1} utilise {2}!", speciesname, GameData::Move.get(move).name))
...
HiddenMoveHandlers::CanUseMove.add(:CUT, proc { |move, pkmn, showmsg|
  ...
  if !facingEvent || !facingEvent.name[/cuttree/i]
    pbMessage(_INTL("Impossible ici.")) if showmsg
    next false
  end
  next true
})
...
HiddenMoveHandlers::UseMove.add(:CUT, proc { |move, pokemon|
  if !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("{1} utilise {2}!", pokemon.name, GameData::Move.get(move).name))
  end
  $stats.cut_count += 1
  facingEvent = $game_player.pbFacingEvent
  if facingEvent
...
HiddenMoveHandlers::CanUseMove.add(:DIG, proc { |move, pkmn, showmsg|
  ...
  if !$game_player.can_map_transfer_with_follower?
    pbMessage(_INTL("Impossible de l'utiliser lorsque vous êtes accompagné.")) if showmsg
    next false
  end
  next true
})
...
def pbDive
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("Les eaux sont profondes ici. Un Pokémon peut être capable de plonger."))
    return false
  end
  if pbConfirmMessage(_INTL("Les eaux sont profondes ici. Voulez-vous utiliser Plongée?"))
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1} utilise {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbFadeOutIn {
...
def pbSurfacing
  ...
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("La lumière filtre d'en haut. Un Pokémon pourrait faire surface ici."))
    return false
  end
  if pbConfirmMessage(_INTL("La lumière filtre d'en haut. Voulez-vous utiliser Plongée?"))
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1} utilise {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbFadeOutIn {
...
HiddenMoveHandlers::CanUseMove.add(:FLASH, proc { |move, pkmn, showmsg|
  ...
  if $PokemonGlobal.flashUsed
    pbMessage(_INTL("Flash est déjà utilisé.")) if showmsg
    next false
  end
  next true
})
...
def pbCanFly?(pkmn = nil, show_messages = false)
  ...
  if !$game_player.can_map_transfer_with_follower?
    pbMessage(_INTL("Impossible de l'utiliser lorsque vous êtes accompagné.")) if show_messages
    return false
  end
...
def pbFlyToNewLocation(pkmn = nil, move = :FLY)
  ...
  if !pkmn || !pbHiddenMoveAnimation(pkmn)
    ...
    pbMessage(_INTL("{1} utilise {2}!", name, GameData::Move.get(move).name))
  end
  $stats.fly_count += 1
  pbFadeOutIn {
    pbSEPlay("Fly")
...
def pbHeadbutt(event = nil)
  ...
  movefinder = $player.get_pokemon_with_move(move)
  if !$DEBUG && !movefinder
    pbMessage(_INTL("Un Pokémon pourrait être dans cet arbuste. Un Pokémon pourrait le secouer."))
    return false
  end
  if pbConfirmMessage(_INTL("Un Pokémon pourrait être dans cet arbuste. Voulez-vous utiliser Coup de Boule?"))
...
def pbRockSmash
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("C'est un rocher robuste, mais un Pokémon peut être capable de le briser."))
    return false
  end
  if pbConfirmMessage(_INTL("Ce rocher semble brisable. Voulez-vous utiliser Eclate-Roc?"))
...
def pbStrength
  if $PokemonMap.strengthUsed
    pbMessage(_INTL("Force permet de déplacer les gros rochers."))
    return false
  end
  move = :STRENGTH
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("C'est un gros rocher, mais un Pokémon peut être capable de le pousser."))
    return false
  end
  pbMessage(_INTL("C'est un gros rocher, mais un Pokémon peut être capable de le pousser.\1"))
  if pbConfirmMessage(_INTL("Voulez-vous utiliser Force?"))
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1} utilise {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbMessage(_INTL("La force de {1} permet de déplacer les gros rochers!",speciesname))
...
HiddenMoveHandlers::CanUseMove.add(:STRENGTH, proc { |move, pkmn, showmsg|
  ...
  if $PokemonMap.strengthUsed
    pbMessage(_INTL("Force est déjà utilisé.")) if showmsg
    next false
  end
  next true
})
...
HiddenMoveHandlers::UseMove.add(:STRENGTH, proc { |move, pokemon|
  if !pbHiddenMoveAnimation(pokemon)
    pbMessage(_INTL("{1} utilise {2}!\1", pokemon.name, GameData::Move.get(move).name))
  end
  pbMessage(_INTL("La force de {1} permet de déplacer les gros rochers!",pokemon.name))
  $PokemonMap.strengthUsed = true
  next true
})
...
def pbSurf
  ...
  if pbConfirmMessage(_INTL("L'eau est tintée d'un bleu profond...\nVoulez-vous utiliser Surf?"))
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1} utilise {2}!",speciesname,GameData::Move.get(move).name))
    pbCancelVehicles
    pbHiddenMoveAnimation(movefinder)
...
HiddenMoveHandlers::CanUseMove.add(:SURF, proc { |move, pkmn, showmsg|
  ...
  if $PokemonGlobal.surfing
    pbMessage(_INTL("Vous surfez déjà.")) if showmsg
    next false
  end
  ...
  if $game_map.metadata&.always_bicycle
    pbMessage(_INTL("Appréciez le vélo!")) if showmsg
    next false
  end
  if !$game_player.pbFacingTerrainTag.can_surf_freely ||
     !$game_map.passable?($game_player.x, $game_player.y, $game_player.direction, $game_player)
    pbMessage(_INTL("Surf est interdit ici!")) if showmsg
    next false
  end
  next true
})
...
def pbSweetScent
  if $game_screen.weather_type != :None
    pbMessage(_INTL("Le doux parfum s'est estompé pour une raison quelconque..."))
    return
  end
  ...
  if !enctype || !$PokemonEncounters.encounter_possible_here? ||
    ...
    pbMessage(_INTL("Il semble qu'il n'y ait rien ici..."))
  end
end
...
HiddenMoveHandlers::ConfirmUseMove.add(:TELEPORT, proc { |move, pkmn|
  ...
  next pbConfirmMessage(_INTL("Vous voulez retourner au dernier lieu de soin utilisé à {1}?", mapname))
})
...
def pbWaterfall
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_WATERFALL, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("Un mur d'eau se brise dans un puissant vrombissement."))
    return false
  end
  if pbConfirmMessage(_INTL("C'est une grande chute d'eau. Voulez-vous utiliser Cascade?"))
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1} utilise {2}!", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbAscendWaterfall
